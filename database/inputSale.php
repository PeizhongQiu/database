    ...
    <div class="form_input">
            ...
            <fieldset>
                ...
                <table id="buy" width=100% >
                    ...
                    <?php
                        ...
                        $sql = "select * from SALE_NOT_SUB WHERE SID = '" . $_SESSION['id'] . "'"; //sql语句写在这
        ...
        <?php 
            if( $_POST['submit'] == "保存"){
                ...
                $server = new sql('LAPTOP-BF7H3R0J','sale','123456','last');
                $server->transaction();
                $sql = "DELETE FROM SALE_NOT_SUB WHERE SID = ? ";
                ...
                for($i=1;$i<=$all/2;$i++){
                    ...
                    //var_dump($param);
                    $sql = "INSERT INTO SALE_NOT_SUB VALUES(?,?,?)"; //sql语句写在这
                ...
                $eff=0;
                $good_num='S'.substr('0000000000'.$num,strlen('0000000000'.$num)-9);

                $param=array($good_num,$_SESSION['id'],date("Y-m-d"));
                $sql = "INSERT INTO SALE (ID, SID, STIME, STATE) VALUES (?, ?,?,0)";
                $result = $server->doQuery_2( $sql,$param);
                if ( !is_string( $result ) && $result){
                ...
                for($i=1;$i<=$all/2;$i++){
                    ...
                    if ( !is_string( $result ) && $result){
                        while ( $re = sqlsrv_fetch_array ( $result )){
                            $num=(int)($re['NUM'])-(int)($_POST['NUM'.$i]);
                            //echo $num;
                        }
                        if($num<0){
                            echo $_POST['GID'.$i].'缺货';
                    ...
                    $sql = "INSERT INTO SALE_NUM VALUES(?,?,?)"; //sql语句写在这
                    $result = $server->doQuery_2( $sql,$param); //查询返回的句柄
                    if ( !is_string( $result ) && $result){
                        //echo "添加成功";
                        $eff++;
